...
void Model::computeOutput(Vector& hidden, Vector& output) const {
  if (args_->model == model_name::xcbow) {
    if (quant_) {
      std::cerr << "xcbow don't support quant" << std::endl;
      abort();
    } else {
      int m = static_cast<int>(wo_->m_);
      int n = static_cast<int>(wo_->n_);
      real alpha = 1.0;
      real *a = wo_->data_;
      int lda = m;
      real *x = hidden.data_;
...
// for predictions
void Model::computeOutput() {
  computeOutput(hidden_, output_);
}
...
void Model::computeHidden(const std::vector<int32_t>& input, Vector& hidden) const {
  ...
  if (args_->model == model_name::pctr) {
    Vector mean(args_->dim);
    mean.zero();
    ...
    if (input.size() > 1) {
      mean.mul(1.0 / (input.size() - 1));
    }

    hidden.concat(mean, *wi_, *input.cbegin());
  } else {
...
void Model::pctrPredict(const std::vector<int32_t>& input, int32_t k,
                    std::vector<std::pair<real, int32_t>>& heap) {
  pctrPredict(input, k, heap, hidden_, output_);
}
void Model::pctrPredict(const std::vector<int32_t>& input, int32_t k,
                    std::vector<std::pair<real, int32_t>>& heap,
                    Vector& hidden, Vector& output) const {
...
void Model::update(const std::vector<int32_t>& input, int32_t target, real lr) {
  ...
  if (args_->model == model_name::pctr) {
    Vector grad_left(args_->dim);
    Vector grad_right(args_->dim);
    grad_.split(grad_left, grad_right);

    for (auto it = input.cbegin() + 1; it != input.cend(); ++it) {
      // grad_ left half
      wi_->addRow(grad_left, *it, 1.0);
    }

    // grad_ right half
    wi_->addRow(grad_right, *input.cbegin(), 1.0);
  } else {
    for (auto it = input.cbegin(); it != input.cend(); ++it) {
